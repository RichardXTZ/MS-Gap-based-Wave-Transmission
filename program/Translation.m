function [result_population] = Translation(current_population,gene_length,R_lim,d_lim,wn_lim,dc_lim,theta_lim,prop_lim,resolution)

result_population=zeros(size(current_population,1),size(current_population,2)/gene_length);

lim = [repmat(R_lim,1,1); repmat(d_lim,2,1);repmat(wn_lim,2,1); repmat(dc_lim,1,1);  repmat(theta_lim,1,1)];
